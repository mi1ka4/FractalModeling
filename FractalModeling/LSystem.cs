...
namespace FractalModeling
{
    public class LSystem
    {
            ...
            string sentence, float startX, float startY, float startAngleDeg)
        {
            ...
            float x = startX;
            float y = startY;
            float angle = (float)(startAngleDeg * Math.PI / 180f);
            float step = StepSize;
            float dRad = (float)(Delta * Math.PI / 180f);
            ...
            foreach (char c in sentence)
            {
                ...
                switch (c)
                {
                    ...
                    case 'B':
                        {
                            float nx = (float)(x + s * Math.Cos(angle));
                            float ny = (float)(y - s * Math.Sin(angle)); // Y-ось экрана
                    ...
                    case 'f':
                        x += s * (float)(Math.Cos(angle));
                        y -= s * (float)(Math.Sin(angle));
                        break;
                    case '+': angle += dRad; break;
                    case '-': angle -= dRad; break;
                    case '|': angle += (float)Math.PI; break;
        ...
        /// <summary>
        /// Вычисляет bounding box при StepSize=1 для автомасштабирования.
        /// </summary>
        ...
        {
            var w = new System.IO.StreamWriter(path, false, Encoding.UTF8);
